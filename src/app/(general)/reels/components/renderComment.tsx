...
import InputComment from "@/components/post/detail-post/comment/input-comment";
...
export default function RenderComment({ data, comments, myUser, maxHeight }: CommentProps) {
    ...
    const handleLoadMoreComments = useCallback(async () => {
        ...
        try {
            const { data: dataComment } = await action_GetCommentBySlugPost(data?.post?.slug, page + 1, 5);
    ...
    const handleHideReplyComment = useCallback((id: string) => {
    ...
    const renderOneComment = useCallback((comment: CommentType, depth = 0) => {
        return (
    ...
    }, [dataReply, handleSetReply, handleHideReplyComment, handleGetReplyComment, textColorPrimary, textColorSecondary, borderColor, lang]);
    ...
    return (
        <>
            <Box id="comment-container"
                sx={{
                    width: '100%', height: 629, overflowY: 'auto', padding: '10px',
                    borderBottom: `0.5px solid ${textColorSecondary}`
                }}>
