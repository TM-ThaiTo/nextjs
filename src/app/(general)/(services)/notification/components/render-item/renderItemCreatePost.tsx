...
export default function RenderItemNotificationCreatePost({ data, notification }: Props) {
    ...
    const renderTypePost = (type: number) => {
        return (
            <>
                {type === 1 && <>{locales[lang]?.notificationPage?.post?.newImage}</>}
                {type === 2 && <>{locales[lang]?.notificationPage?.post?.newVideo}</>}
                {type === 3 && <>{locales[lang]?.notificationPage?.post?.newStatus}</>}
            </>
        )
    }
        ...
        <Box onClick={handleIsRead} sx={{ height: 80, width: '100%', display: 'flex', alignItems: 'center', p: 1, cursor: 'pointer', '&:hover': { backgroundColor: actionHoverColor } }} >
            ...
            <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', ml: 1, width: '90%' }}>
                <Box sx={{ height: 'auto', maxHeight: 65, display: 'flex', alignItems: 'center' }}>
                    ...
                    <Typography sx={{ fontSize: 16, fontWeight: 500, color: !isReadNotification ? textColorPrimary : textColorSecondary, ml: 1 }}>{renderTypePost(typePost)}  {content}</Typography>
                </Box>
