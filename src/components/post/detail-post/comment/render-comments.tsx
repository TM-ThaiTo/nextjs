...
export default function RenderCommentDetailPost({ data, comments, myUser, maxHeight }: CommentProps) {
                ...
                <Box sx={{ display: 'flex', marginBottom: '5px' }}>
                    <Box sx={{ width: 40, height: 40 }}>
                        <Link href={`/${comment?.user?.slug}`}>
                            <Avatar src={comment?.user?.avatar || '/static/avt_default.png'} alt={'User'} sx={{ height: 32, width: 32, objectFit: 'cover', border: `0.5px solid ${borderColor}` }} />
                        </Link>
                    </Box>
    ...
    const renderNoAuth = () =>
        <Box sx={{ height: 60, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <Typography component="span" variant="body2" sx={{ ml: 2, color: textColorPrimary }}>
                {locales[lang]?.comment?.likeOrComment}
            </Typography>
        </Box>
    ...
    return (
        <>
            <Box id="comment-container"
                sx={{ width: '100%', height: '100%', overflowY: 'scroll', padding: '10px', borderBottom: `0.5px solid ${borderColor}` }}>
                <RenderContent data={data} lang={lang} />
                {listComments.length > 0 ? (
                    listComments.map((item: any, index: any) => (
                        <Fragment key={index}>{renderOneComment(item)}</Fragment>
                    ))
                ) : <Box sx={{ height: 140, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>{locales[lang]?.comment?.noComment}</Box>}
                {loadingGetComments && <Box sx={{ textAlign: 'center', padding: '10px 0' }}> {locales[lang]?.loading} </Box>}
            </Box>

            <Box style={{ width: '100%', height: 140, borderBottom: `1px solid ${borderColor}` }}>
                <Box sx={{ paddingLeft: '10px', height: 80 }}>
