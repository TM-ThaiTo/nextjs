...
const RenderImage = memo(({ listUrl, h, w, o }: RenderImageProps) => {
    const sliderRef = useRef<Slider>(null);
    const current = listUrl?.length;
                    ...
                    {listUrl?.length === 1 ? (
                        <Box sx={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'black' }}>
                            <Box
                                ...
                                src={listUrl[0]?.url}
                                alt="My Image"
                                sx={{ width: w, height: h, objectFit: o as any }}
                            />
                        </Box>
                    ) : (
                        <Box sx={{ height: '100%', width: '100%' }}>
                            ...
                            >
                                {listUrl.map((image, index) => (
                                    <Box key={index}
                                        sx={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'black' }}>
                                        <Box
                                            component="img"
                                            src={image.url}
                                            alt={`Image ${index}`}
                                            sx={{ height: h, width: w, objectFit: o as any }}
                                        />
                                    </Box>
                                ))}
                            </Slider>
