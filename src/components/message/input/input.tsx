...
import { createMessage } from '@/actions/chat/p2p/message/actions';
...
export default function InputTextField({ idRoom, idReceiver }: InputTextFieldProps) {
    ...
    const onSubmit = useCallback(async () => {
        ...
        formData.append('receiver', idReceiver);
        ...
        if (error) { setSnackbarMessage({ type: 'error', message: 'Đã xãy ra lỗi, vui lòng thử lại sau' }); setOpenSnackbar(true); return; }
        setMessage('');
        setFiles([]);
    }, [message, idRoom, idReceiver, files, setOpenSnackbar, setSnackbarMessage]);
                        ...
                        <Box key={index} sx={{ position: 'relative', marginRight: '10px', marginBottom: '10px' }}>
                            {file.type.startsWith('image/')
                                ...
                                : file.type.startsWith('video/')
                                    ? <video src={URL.createObjectURL(file)} width={200} height={200} controls style={StyleImage} />
